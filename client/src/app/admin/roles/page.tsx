...
const createColumns = (tableActions: ReturnType<typeof useTableActions<RoleList>>,
    onRegisterFingerprint?: (ronl: RoleList) => void): ColumnDef<RoleList>[] => [
        {
            accessorKey: "id",
                ...
                return (
                    <TableActionsMenu
                        ...
                        extraActionsBeforeEdit={[
                            {
                                label: "Inabilitar Rol",
                                icon: <EyeOff className="h-4 w-4" />,
                                onClick: (r: RoleList) => {
                                    if (onRegisterFingerprint) return onRegisterFingerprint(r);
                                },
                                show: (v: RoleList) => v.activo,
                                separator: false,
                            },
                        ]}
                    />
...
export default function RolesPage() {
    ...
    const columns = createColumns(tableActions, (rol: RoleList) => {
        // Aquí puedes implementar la lógica para inabilitar el rol
        console.log(`Inabilitar rol con ID: ${rol.id}`);
        inabilotarRole(rol.id);
        refresh();
    });
