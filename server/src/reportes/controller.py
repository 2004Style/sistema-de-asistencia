...
async def obtener_estadisticas(
    fecha_inicio: Optional[date] = Query(None, description="Fecha inicio (YYYY-MM-DD), default: hace 30 días"),
    fecha_fin: Optional[date] = Query(None, description="Fecha fin (YYYY-MM-DD), default: hoy"),
    db: Session = Depends(get_db),
    ...
    Retorna:
    - Estadísticas generales (total asistencias, presentes, ausentes, tardías)
    - Usuarios únicos registrados y con asistencias
    - Asistencias por rol
    - Asistencias por turno
    - Tendencia diaria de asistencias
    
    **Rango de fechas:**
    - Si no se especifica: últimos 30 días
    - Si solo fecha_inicio: desde fecha_inicio hasta hoy
    - Si ambas: usa el rango especificado
    """
    try:
        resultado = reportes_service.obtener_estadisticas(
            db=db,
            fecha_inicio=fecha_inicio,
            fecha_fin=fecha_fin
        )
        ...
        if not resultado.get("success"):
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=resultado.get("error", "Error al obtener estadísticas")
            )
        
        return {
            "success": True,
            "data": resultado,
            "consultado_por": current_user.name,
            "timestamp": datetime.now().isoformat()
        }
