...
class EmailService:
    """
    Service for sending emails via external Mail API with support for templates and attachments.
    Similar to nodemailer in Node.js, but using HTTP-based Mail API instead of SMTP.
    """
    
    def __init__(self):
        # Mail API configuration
        self.mail_api_url = settings.MAIL_API_URL.rstrip("/") if settings.MAIL_API_URL else None
        self.mail_api_client_id = settings.MAIL_API_CLIENT_ID
        self.mail_api_secret = settings.MAIL_API_SECRET
