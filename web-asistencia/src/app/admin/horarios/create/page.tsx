...
import { CrearHorario, TurnosList } from "@/interfaces"
...
export default function HorarioCreatePage() {
    ...
    useEffect(() => {
        let mounted = true
            ; (async () => {
                try {
                    const resp = await get("/turnos")
                    if (resp.alert === "success" && resp.data) {
                        // resp.data puede ser { turnos: [...] } o un array según backend
                        const data: any = resp.data
                        const lista = Array.isArray(data.turnos) ? data.turnos : data
                        if (mounted) setTurnos(lista)
                    }
                } catch (e) {
